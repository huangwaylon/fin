...
const SECTOR_MASKS = {
  financial: ['evEbitda', 'netDebtToEbitda', 'currentRatio', 'interestCoverage', 'debtToEquity', 'fcfYield', 'fcfMargin', 'fcf'],
  realestate: ['earningsYield', 'peg', 'netDebtToEbitda', 'debtToEquity'],
};
...
export function computeFactors(exRaw, weightsOverride) {
  ...
  const quality = factor([
    ['roe', 'Return on equity', ex.roe, scoreUp(ex.roe, 0, 0.3), 0.25],
    ['roa', 'Return on assets', ex.roa, scoreUp(ex.roa, 0, 0.15), 0.15],
    ['grossMargin', 'Gross margin', ex.grossMargin, scoreUp(ex.grossMargin, 0.1, 0.6), 0.15],
    ['operatingMargin', 'Operating margin', ex.operatingMargin, scoreUp(ex.operatingMargin, 0, 0.3), 0.15],
    ['netMargin', 'Net margin', ex.netMargin, scoreUp(ex.netMargin, 0, 0.25), 0.15],
    ['fcfMargin', 'FCF margin', ex.fcfMargin, scoreUp(ex.fcfMargin, 0, 0.25), 0.15],
  ]);

  const value = factor([
    ['earningsYield', 'Earnings yield', ex.earningsYield, scoreUp(ex.earningsYield, 0, 0.1), 0.3],
    ['fcfYield', 'FCF yield', ex.fcfYield, scoreUp(ex.fcfYield, 0, 0.08), 0.25],
    ['evEbitda', 'EV/EBITDA', ex.evEbitda, scoreDown(ex.evEbitda, 6, 30), 0.2],
    ['pb', 'Price/Book', ex.pb, scoreDown(ex.pb, 1, 12), 0.1],
    ['ps', 'Price/Sales', ex.ps, scoreDown(ex.ps, 1, 15), 0.1],
