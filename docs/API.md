...
| `GET /api/backtest` | `symbol`, `rule?`, `years?` | Walk-forward price-signal backtest vs buy-and-hold |

`weights` overrides factor weights, e.g. `weights=quality:0.3,value:0.25,growth:0.2`.
`rule` ∈ `trend_mom` (default) · `trend` · `mom` · `buyhold`.
...
```jsonc
{
  ...
  "valuation": {
    ...
    "impliedGrowth": 0.264, "analystUpside": 0.118, "sensitivity": [ … ]
  },
  "signals": { "trend": "up", "mom12_1": …, "annualVol": …, "maxDrawdown": … },
  "fundamentals": { … }, "flags": [ … ], "bull": [ … ], "bear": [ … ],
